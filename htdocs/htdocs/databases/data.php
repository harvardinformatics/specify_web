...
if (preg_match("/^\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/", $uuid)) {
	$query = "select f.collectionobjectid from guids g, fragment f where f.fragmentid = g.primarykey and g.tablename='fragment' and g.uuid=?";
...
if (preg_match("/^\d{1,8}$/", $barcode)) {
	...
	$query = "select f.collectionobjectid as coid from fragment f where f.identifier = ?
	union
