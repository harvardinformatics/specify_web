...
function findOrCreateSet($batchid,$collectionobjectid, $path, $filename) {
    ...
    $result = null;
    $fullpath = "$mountpointparent$path";
    $set_identifier = preg_replace('/\.\w+$/', '', $path . $filename);
    ...
    if ($statement->prepare($sql)) {
       ...
       $statement->bind_param("s",$set_identifier);
       if ($statement->execute()) {
           ...
           if ($statement->num_rows>0) {
              ...
              $remarks = "";
              $sql = "insert into IMAGE_SET (specimen_id,batch_id,access_type_id,source_type_id,active_flag,owner,copyright,remarks,set_identifier) values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
              if ($debug) { echo "$sql\n"; }
              $stmtinsert = $connection->prepare($sql);
              $stmtinsert->bind_param('iiiiissss',$collectionobjectid,$batchid,$accesstypeid,$sourcetypeid,$activeflag,$owner,$copyright,$remarks,$set_identifier);
              if ($stmtinsert->execute()) {
                 if ($stmtinsert->affected_rows==1) {
                    $result = $connection->insert_id;
                 }
