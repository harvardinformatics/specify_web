...
then
 echo ERROR: Problem with JPEGs: $COUNT_JPG doesnt match $COUNT_CR2
 exit 1
fi
...
then
 echo ERROR: Problem with JPEG Previews: $COUNT_JPG_PREV doesnt match $COUNT_CR2
 exit 1
fi
...
then
 echo ERROR: Problem with JPEG Thumbnails: $COUNT_JPG_THUMB doesnt match $COUNT_CR2
 exit 1
fi
...
then
 echo ERROR: Problem with DNGs: $COUNT_DNG doesnt match $COUNT_CR2
 exit 1
else
