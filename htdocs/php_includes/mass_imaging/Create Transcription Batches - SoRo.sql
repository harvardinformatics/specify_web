...
insert into TR_BATCH (path, image_batch_id) 
  select concat('SoRo: ', path), image_batch_id from TR_BATCH where image_batch_id not in (select image_batch_id from TR_BATCH where image_batch_id is not null and path like 'SoRo%')
;
